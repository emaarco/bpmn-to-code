...
class TypeScriptProcessApiBuilderTest {
    ...
    fun `buildApiFile generates variant-scoped Flows and Relations for merged model`() {
        ...
        val modelApi = BpmnModelApi(merged, OutputLanguage.TYPESCRIPT, "de.emaarco.example", ProcessEngine.ZEEBE)

        // when
        val result = underTest.buildApiFile(modelApi)

        // then
        val expectedFile = File(requireNotNull(javaClass.getResource("/api/MultiVariantProcessApiTypeScript.txt")).toURI())
        assertThat(result.content).isEqualToIgnoringWhitespace(expectedFile.readText())
    }
}

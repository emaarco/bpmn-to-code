...
import io.github.emaarco.bpmn.adapter.outbound.codegen.emitter.JavaFileEmitter
...
class JavaProcessApiBuilder : CodeGenerationAdapter.AbstractProcessApiBuilder<TypeSpec.Builder>() {
    ...
    override fun buildApiFile(modelApi: BpmnModelApi): GeneratedApiFile {
        ...
        val rootBuilder = TypeSpec.Builder(className)
        ...
        val content = JavaFileEmitter.emit(fileSpec)
    ...
    private inner class ProcessIdWriter : ObjectWriter<TypeSpec.Builder> {
        ...
        override fun write(builder: TypeSpec.Builder, modelApi: BpmnModelApi) {
            builder.addProperty(typedField("PROCESS_ID", PROCESS_ID, modelApi.model.processId))
        }
    }
    ...
    private inner class ElementsWriter : ObjectWriter<TypeSpec.Builder> {
        ...
        override fun write(builder: TypeSpec.Builder, modelApi: BpmnModelApi) {
            ...
            modelApi.model.flowNodes.forEach { flowNode ->
                elementsBuilder.addProperty(typedField(flowNode.getName(), ELEMENT_ID, flowNode.getValue()))
            }
    ...
    private inner class FlowsWriter : ObjectWriter<TypeSpec.Builder> {
        ...
        override fun write(builder: TypeSpec.Builder, modelApi: BpmnModelApi) {
            builder.addType(buildFlowsClass(modelApi.model.sequenceFlows))
        }
    }
    ...
    private inner class RelationsWriter : ObjectWriter<TypeSpec.Builder> {
        ...
        override fun write(builder: TypeSpec.Builder, modelApi: BpmnModelApi) {
            builder.addType(buildRelationsClass(modelApi.model.flowNodes))
        }
    }
    ...
    private inner class VariantsWriter : ObjectWriter<TypeSpec.Builder> {
        ...
        override fun write(builder: TypeSpec.Builder, modelApi: BpmnModelApi) {
            ...
            model.variants.forEach { variant -> variantsBuilder.addType(buildVariantClass(variant)) }
    ...
    private fun buildFlowInitializer(flow: SequenceFlowDefinition): InitializerSpec {
        val name = flow.flowName?.let { javaString(it) } ?: "null"
        val condition = flow.conditionExpression?.let { javaString(it) } ?: "null"
        val args = listOf(
            javaString(flow.id ?: ""),
            name,
            javaString(flow.sourceRef),
            javaString(flow.targetRef),
            condition,
            flow.isDefault.toString(),
        )
        return InitializerSpec.ofSingle("new BpmnFlow(${args.joinToString(", ")})")
    }
    ...
    private fun buildRelationsClass(flowNodes: List<FlowNodeDefinition>): TypeSpec {
        ...
        flowNodes
            ...
            .forEach { node ->
                ...
                relationsBuilder.addProperty(PropertySpec(node.getName(), BPMN_RELATIONS, initializer, refs = listOf(BPMN_RELATIONS, LIST)))
            }
        return relationsBuilder.build()
    }

    private fun buildRelationsInitializer(node: FlowNodeDefinition): InitializerSpec {
        val name = node.displayName?.let { javaString(it) } ?: "null"
        val parentId = node.parentId?.let { javaString(it) } ?: "null"
        val attachedToRef = node.attachedToRef?.let { javaString(it) } ?: "null"
        val args = listOf(
            name,
            javaListLiteral(node.previousElements),
            javaListLiteral(node.followingElements),
            parentId,
            attachedToRef,
            javaListLiteral(node.attachedElements),
        )
        return InitializerSpec.ofSingle("new BpmnRelations(${args.joinToString(", ")})")
    }

    private fun javaListLiteral(items: List<String>): String {
        if (items.isEmpty()) return "List.of()"
        return "List.of(${items.joinToString(", ") { javaString(it) }})"
    }
    ...
    private inner class CallActivitiesWriter : ObjectWriter<TypeSpec.Builder> {
        ...
        override fun write(builder: TypeSpec.Builder, modelApi: BpmnModelApi) {
            ...
            modelApi.model.callActivities.forEach { callActivity ->
                callActivitiesBuilder.addProperty(typedField(callActivity.getName(), PROCESS_ID, callActivity.getValue()))
            }
    ...
    private inner class MessagesWriter : ObjectWriter<TypeSpec.Builder> {
        ...
        override fun write(builder: TypeSpec.Builder, modelApi: BpmnModelApi) {
            ...
            modelApi.model.messages.forEach { message ->
                messagesBuilder.addProperty(typedField(message.getName(), MESSAGE_NAME, message.getValue()))
            }
    ...
    private inner class ServiceTasksWriter : ObjectWriter<TypeSpec.Builder> {
        ...
        override fun write(builder: TypeSpec.Builder, modelApi: BpmnModelApi) {
            ...
            modelApi.model.serviceTasks
                ...
                .forEach { task ->
                    val initializer = InitializerSpec.ofSingle(javaString(task.getValue()))
                    tasksBuilder.addProperty(PropertySpec(task.getName(), STRING, initializer))
                }
    ...
    private inner class SignalsWriter : ObjectWriter<TypeSpec.Builder> {
        ...
        override fun write(builder: TypeSpec.Builder, modelApi: BpmnModelApi) {
            ...
            modelApi.model.signals.forEach { signal ->
                signalsBuilder.addProperty(typedField(signal.getName(), SIGNAL_NAME, signal.getValue()))
            }
    ...
    private inner class VariablesWriter : ObjectWriter<TypeSpec.Builder> {
        ...
        override fun write(builder: TypeSpec.Builder, modelApi: BpmnModelApi) {
            ...
            for (node in nodesWithVariables) {
                ...
                for (rawName in variablesByName.keys.sorted()) {
                    ...
                    nodeVarsBuilder.addProperty(createDirectionalField(group.first(), subtype))
                }
        ...
        private fun createDirectionalField(variable: VariableDefinition, subtype: VariableNameSubtype): PropertySpec {
            val type = VARIABLE_NAME.copy(nested = subtype.simpleName)
            val initializer = InitializerSpec.ofSingle("new ${type.referencedName()}(${javaString(variable.getValue())})")
    ...
    private inner class ErrorsWriter : ObjectWriter<TypeSpec.Builder> {
        ...
        override fun write(builder: TypeSpec.Builder, modelApi: BpmnModelApi) {
            ...
            modelApi.model.errors.forEach {
                ...
                val initializer = InitializerSpec.ofSingle("new BpmnError(${javaString(errorName)}, ${javaString(errorCode)})")
    ...
    private inner class EscalationsWriter : ObjectWriter<TypeSpec.Builder> {
        ...
        override fun write(builder: TypeSpec.Builder, modelApi: BpmnModelApi) {
            ...
            modelApi.model.escalations.forEach {
                ...
                val initializer = InitializerSpec.ofSingle("new BpmnEscalation(${javaString(escalationName)}, ${javaString(escalationCode)})")
    ...
    private inner class CompensationsWriter : ObjectWriter<TypeSpec.Builder> {
        ...
        override fun write(builder: TypeSpec.Builder, modelApi: BpmnModelApi) {
            ...
            modelApi.model.compensations.forEach { compensation ->
                compensationsBuilder.addProperty(typedField(compensation.getName(), ELEMENT_ID, compensation.getValue()))
            }
    ...
    private inner class TimersWriter : ObjectWriter<TypeSpec.Builder> {
        ...
        override fun write(builder: TypeSpec.Builder, modelApi: BpmnModelApi) {
            ...
            modelApi.model.timers.forEach { timer ->
                ...
                val initializer = InitializerSpec.ofSingle("new BpmnTimer(${javaString(timerType)}, ${javaString(timerValue)})")
    ...
    private fun typedField(name: String, type: ClassRef, value: String): PropertySpec {
        val initializer = InitializerSpec.ofSingle("new ${type.referencedName()}(${javaString(value)})")
        return PropertySpec(name, type, initializer)
    }

    /** Mirrors JavaPoet's `$S`: a double-quoted Java string literal with escaped backslashes/quotes. */

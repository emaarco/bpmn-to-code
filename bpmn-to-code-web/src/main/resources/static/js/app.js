...
function renderResults(files, language) {
    ...
    if (files.length > 1) {
        resultsContent.innerHTML += `
            <button class="btn-download btn-download-all" onclick="downloadAllFiles()">
...
async function downloadAllFiles() {
    ...
    // For a proper ZIP, would need a library like JSZip
    for (let i = 0; i < state.generatedFiles.length; i++) {
        await new Promise(resolve => setTimeout(resolve, 200)); // Small delay between downloads
        downloadFile(state.generatedFiles[i].fileName, i);
    }
}

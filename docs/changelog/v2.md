# v2.0.0 Changelog

## Unreleased — Directional Variables

### Variables split into `Inputs` / `Outputs`

Every `Variables.<Element>` object now carries nested `Inputs` / `Outputs` sub-objects that reflect the direction the variable flows in the source BPMN. Consumers (including AI agents reasoning about a process) can statically tell whether a variable is read by an element or produced by it.

```kotlin
// Before
ProcessApi.Variables.ActivitySendConfirmationMail.SUBSCRIPTION_ID

// After
ProcessApi.Variables.ActivitySendConfirmationMail.Inputs.SUBSCRIPTION_ID
ProcessApi.Variables.StartEventSubmitRegistrationForm.Outputs.SUBSCRIPTION_ID
```

Empty sub-objects are never emitted — an element with inputs only emits `Inputs`, and vice versa. See [ADR 015](/contributing/adr/015-directional-variable-extraction) for the design rationale.

### `additionalVariables` replaced by directional variants

The legacy undirected `camunda:property name="additionalVariables"` extension property is no longer extracted. Two directional replacements are supported on any BPMN element:

```xml
<bpmn:extensionElements>
  <camunda:properties>
    <camunda:property name="additionalInputVariables" value="orderId, customerEmail" />
    <camunda:property name="additionalOutputVariables" value="processingResult" />
  </camunda:properties>
</bpmn:extensionElements>
```

**Migration**: search your BPMN files for `name="additionalVariables"` and split each into the two directional variants. The `migrate-bpmn-to-code-v1-to-v2` skill flags BPMN occurrences and `Variables.<Element>.X` source references for manual review but cannot rewrite BPMN files automatically.

---

## What's New

### New API Sections

v2.0.0 adds four new sections to every generated Process API. They are **additive** — existing code continues to compile without changes.

| Section | Description |
|---------|-------------|
| `Flows` | Sequence flow definitions with `id`, optional `name`, `sourceRef`, `targetRef`, optional `condition`, and `isDefault` |
| `Relations` | Per-element topology: optional `name`, `previousElements`, `followingElements`, `parentId`, `attachedToRef`, `attachedElements` |
| `Escalations` | Escalation definitions with `name` and `code` (`BpmnEscalation`) |
| `Compensations` | Compensation event IDs |

### Shared Types in a `types/` Package

`BpmnTimer`, `BpmnError`, and the new `BpmnEscalation`, `BpmnFlow`, `BpmnRelations` are now generated as standalone files under `{packagePath}/types/` instead of being nested inside the process API. This eliminates duplicate type definitions when multiple processes share a package.

### Typed Leaf Identifiers

Leaf constants across `PROCESS_ID`, `Elements`, `CallActivities`, `Messages`, `Compensations`, `Signals`, and `Variables` are now wrapped in type-safe classes so the compiler can catch category mix-ups. Kotlin emits `@JvmInline value class` types (zero runtime cost) and Java emits `record` types.

| Wrapper | Applies to |
|---------|------------|
| `ProcessId` | `PROCESS_ID`, `CallActivities.*` |
| `ElementId` | `Elements.*`, `Compensations.*` |
| `MessageName` | `Messages.*` |
| `SignalName` | `Signals.*` |
| `VariableName` | `Variables.*.*` |

`ServiceTasks.*` and `PROCESS_ENGINE` remain as `const val String` / `public static final String` because Kotlin/Java annotation arguments (e.g. `@JobWorker(type = ...)`) require compile-time constants.

Consumers unwrap with `.value` (Kotlin) or `.value()` (Java record), or update downstream signatures to accept the wrapper type.

### Variables Nested Per Element

Variables are now grouped by the element they belong to, eliminating duplicate entries when the same variable name appears in multiple tasks.

## Breaking Changes

### 1. `TaskTypes` renamed to `ServiceTasks`

```kotlin
// v1.1.0
ProcessApi.TaskTypes.SEND_CONFIRMATION_MAIL

// v2.0.0
ProcessApi.ServiceTasks.SEND_CONFIRMATION_MAIL
```

### 2. `BpmnTimer` and `BpmnError` are now standalone imports

Previously these types were nested inside the process API object. They are now generated in a `types/` sub-package and must be imported directly.

::: code-group

```kotlin [Kotlin — v1.1.0]
val timer: NewsletterSubscriptionProcessApi.Timers.BpmnTimer =
    NewsletterSubscriptionProcessApi.Timers.TIMER_EVERY_DAY
```

```kotlin [Kotlin — v2.0.0]
import de.emaarco.example.types.BpmnTimer

val timer: BpmnTimer = NewsletterSubscriptionProcessApi.Timers.TIMER_EVERY_DAY
```

:::

### 3. Variables are nested per element, then split by direction

Variable constants are now scoped under a sub-object named after the element they belong to **and** split into `Inputs` / `Outputs` sub-objects that reflect the direction they flow in the source BPMN (see the "Unreleased — Directional Variables" section above for details).

```kotlin
// v1.1.0
ProcessApi.Variables.SUBSCRIPTION_ID

// v2.0.0
ProcessApi.Variables.ActivitySendConfirmationMail.Inputs.SUBSCRIPTION_ID
ProcessApi.Variables.StartEventSubmitRegistrationForm.Outputs.SUBSCRIPTION_ID
```

Check the generated API file for the exact element sub-object names and direction.

### 4. Leaf constants are typed wrappers, not `String`

Every leaf constant except `ServiceTasks.*` and `PROCESS_ENGINE` is now a value-class / record instance. Consumers that previously passed them to a `String` parameter must either unwrap or update the receiving signature.

::: code-group

```kotlin [Kotlin — v1.1.0]
engine.sendMessage(name = ProcessApi.Messages.MESSAGE_FORM_SUBMITTED)
```

```kotlin [Kotlin — v2.0.0 (unwrap)]
engine.sendMessage(name = ProcessApi.Messages.MESSAGE_FORM_SUBMITTED.value)
```

```kotlin [Kotlin — v2.0.0 (retype)]
// Preferred: update the callee to accept MessageName
fun sendMessage(name: MessageName) { ... }
engine.sendMessage(name = ProcessApi.Messages.MESSAGE_FORM_SUBMITTED)
```

:::

Wrapped constants cannot be used as annotation arguments (Kotlin and Java require compile-time constants there). If a consumer previously used e.g. `Messages.X` in an annotation, keep a local `const val String` alongside it for the annotation site. `@JobWorker(type = ServiceTasks.X)` continues to work unchanged.

## Migration

Most breaking changes can be applied automatically using the `migrate-bpmn-to-code-v1-to-v2` agent skill.

### With Claude Code (recommended)

Install the skill and run it in your project:

```bash
npx skills add https://github.com/emaarco/bpmn-to-code/tree/main/bpmn-to-code-skills/skills/migrate-bpmn-to-code-v1-to-v2
```

Then invoke it:

```
/migrate-bpmn-to-code-v1-to-v2
```

The skill scans your source files, proposes replacements, and applies them after your approval. Variable path changes (item 3 above) require manual review and are flagged separately.

### Prompt

If you have the skill installed, invoke it directly:

```
/migrate-bpmn-to-code-v1-to-v2
```

### Manual steps

1. Search for `.TaskTypes.` and replace with `.ServiceTasks.`.
2. Search for qualified `BpmnTimer` and `BpmnError` type references — replace with the simple class name and add the `import {pkg}.types.*` statement.
3. For each variable reference (`ProcessApi.Variables.VAR_NAME`), look up the correct per-element path in the regenerated API file.

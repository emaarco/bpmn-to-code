...
| Section | Description |
|---------|-------------|
| `Flows` | Sequence flow definitions with `id`, `sourceRef`, `targetRef`, optional `condition`, and `isDefault` |
...
If you have the skill installed, paste this into your AI assistant:

```
Migrate my project from bpmn-to-code v1 to v2. Scan all source files for deprecated API usage
(TaskTypes → ServiceTasks, nested BpmnTimer/BpmnError types, flat Variables references) and
remove the useVersioning parameter from my build files if present. Show me a migration plan

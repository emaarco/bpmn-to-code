...
The skill scans your build files for removed parameters (e.g. `useVersioning`) and your source files for deprecated API usage — `TaskTypes`, nested `BpmnTimer`/`BpmnError` types, and flat `Variables` references. It proposes all changes before applying them.

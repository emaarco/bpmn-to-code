...
Two complementary tools shrink consumer call sites: **`toString()` lets you drop `.value` entirely** in string-accepting contexts, and **star imports / `with` blocks** collapse the qualified prefix when a file touches many constants from the same element.

**Drop `.value` in string-accepting contexts** — logging, string templates, `println`, any `Any?`-accepting API will call `toString()` on the wrapper:

```kotlin
log.info("sending message {}", ProcessApi.Messages.MESSAGE_FORM_SUBMITTED)
println("variable: ${ProcessApi.Variables.StartEventRequestReceived.SUBSCRIPTION_ID}")
```

**Collapse the qualified prefix** when a file references many constants from one element. `.value` is still required inside these examples because the map types are `Map<String, String>` — this section is about the prefix, not the unwrap:
...
// later:
mapOf(SUBSCRIPTION_ID.value to id.toString())
```
...
// later:
Map.of(SUBSCRIPTION_ID.value(), id.toString());
```
